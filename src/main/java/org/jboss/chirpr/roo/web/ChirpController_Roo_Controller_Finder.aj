// WARNING: DO NOT EDIT THIS FILE. THIS FILE IS MANAGED BY SPRING ROO.
// You may push code into the target .java compilation unit if you wish to edit any member(s).

package org.jboss.chirpr.roo.web;

import java.lang.String;
import org.jboss.chirpr.roo.domain.Profile;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;

privileged aspect ChirpController_Roo_Controller_Finder {
    
    @RequestMapping(params = { "find=ByAuthor", "form" }, method = RequestMethod.GET)
    public String ChirpController.findChirpsByAuthorForm(Model uiModel) {
        uiModel.addAttribute("profiles", Profile.findAllProfiles());
        return "chirps/findChirpsByAuthor";
    }
    
}
